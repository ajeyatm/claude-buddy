...
def validate_and_append_message(messages: list["ChatCompletionMessageParam"], message: dict[str, object]) -> None:
    ...
    validated_msg = validate_message(message)  # Will raise if invalid

    messages.append(cast("ChatCompletionMessageParam", validated_msg))


def validate_message(message: dict[str, object]) -> "ChatCompletionMessageParam":
    """
    Validate message shape and return it as the proper type.
    Does NOT append to any list - just validates and casts.
    """
